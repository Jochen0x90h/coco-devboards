...
const auto txPin = gpio::PA2 | gpio::AF1;
#define UART_INFO uart::USART2_INFO
constexpr auto uartClock = USART2_CLOCK;
...
void __attribute__((weak)) sleep(Microseconds<> time) {
    int64_t count = int64_t(int(SYS_CLOCK) / 737700) * time.value >> 3;
