...
void __attribute__((weak)) SystemInit() {
    ...
    */
    ...
    // configure PLL: 16MHz / 1 * 20 = 320MHz / 2 = 160MHz (set SYS_CLOCK in config.hpp accordingly)
    RCC->PLLCFGR = RCC_PLLCFGR_PLLSRC_HSI // source is internal oscillator HSI16
        | ((1 - 1) << RCC_PLLCFGR_PLLM_Pos) // /M divisor
        | (20 << RCC_PLLCFGR_PLLN_Pos) // *N multiplier
    ...
    RCC->CCIPR = RCC->CCIPR | RCC_CCIPR_ADC12SEL_0 | RCC_CCIPR_ADC345SEL_0;
	...
	// disable dead battery pull-ups (when not using USB and UCPD, otherwise do this after initializing UCPD)
	PWR->CR3 = PWR->CR3 | PWR_CR3_UCPD_DBDIS;

	// enable reference voltage if necessary, this devboard has an external voltage reference configurable by a jumper
